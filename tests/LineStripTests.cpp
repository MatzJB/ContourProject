...
TEST(Line2Tests, GetLineStripValid) {
    ...
    // Get the line strip
    std::vector<Point2> lineStrip = line.getLineStrip();

    // Verify the size of the line strip
    EXPECT_EQ(lineStrip.size(), 2) << "Line strip should contain exactly two points.";

    // Verify the start point
    EXPECT_NEAR(lineStrip[0].x, start.x, EPS) << "Start point X does not match.";
    EXPECT_NEAR(lineStrip[0].y, start.y, EPS) << "Start point Y does not match.";

    // Verify the end point
    EXPECT_NEAR(lineStrip[1].x, end.x, EPS) << "End point X does not match.";
    EXPECT_NEAR(lineStrip[1].y, end.y, EPS) << "End point Y does not match.";
}
